...
## 이전 학습 내용 적용
- `stride`를 사용해서 `startIndex`,`endIndex`를 활용하여 필요한 인덱스 시퀸스만 **지연 생성** 하고 `map`을 체이닝 하여 **사이드 이펙트** 없게 하였다.

## 나의 풀이
```swift
func solution(_ names:[String]) -> [String] {
    stride(from: names.startIndex, to: names.endIndex, by: 5).map { names[$0] }
}
```
