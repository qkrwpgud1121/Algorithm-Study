...
## 배운점
- `reduce(into: [])`를 사용했을때 빈 배열의 타입을 추론하지 못해 애러가 발생하는것을 경험하였다.
- 해서 `reduce(into: [Int]())`와 같이 타입을 명시하여 컴파일 오류를 해결하였다.
...
func solution(_ arr:[Int]) -> [Int] {
    ...
    let result = arr.reduce(into: [Int]()) { stk, num in
        if stk.last == num {
            stk.removeLast()
        } else {
            stk.append(num)
