...
## 배운점
- `reduce(into:)`를 활용하여 `inout` 결과를 직접 갱신하여 연산에 효율성을 극대화 할 수 있었다.
- `map`을 사용하여 데이터와 가격을 1:1로 메칭한뒤 `reduce`를 사용하여 불필요한 조건문과 임시 변수를 제거할 수 있었다.
...
func solution(_ order: [String]) -> Int {
    ...
    return order.reduce(into: 0) { result, element in
