...
## 배운점
- 기존 matrix를 생성해서 대치하는 방법보다 부족한 부분에 추가하는 방법이 성능면에서 더 좋다.
- 삼항연산자를 사용하는 대신 `max`를 사용하여 비교
...
func solution(_ arr: [[Int]]) -> [[Int]] {
    ...
    let width = arr.first?.count ?? 0
    
    let a = width > height ? width : height
    
    var matrix = Array(repeating: Array(repeating: 0, count: a), count: a)
...
func solution(_ arr: [[Int]]) -> [[Int]] {
    ...
    var result = arr.map { row in
        row + Array(repeating: 0, count: maxSide - width)
    }
    
    result += Array(repeating: Array(repeating: 0, count: maxSide), count: maxSide - height)
